 ...
 * 信任证书
 *
 * @author ZhangXiaoMing 2023-10-18 21:28 周三
 */
public class TrustCerts implements X509TrustManager {
     ...
     *                                  parameter or if null or zero-length string is passed in for
     *                                  the authType parameter
     * @throws CertificateException     if the certificate chain is not trusted by this
     *                                  TrustManager.
     */
    @Override
     ...
     * represented as a String, such as "RSA", "DHE_DSS". Note: for some exportable cipher suites,
     * the key exchange algorithm is determined at run time during the handshake. For instance, for
     * TLS_RSA_EXPORT_WITH_RC4_40_MD5, the authType should be RSA_EXPORT when an ephemeral RSA key
     * is used for the key exchange, and RSA when the key from the server certificate is used.
